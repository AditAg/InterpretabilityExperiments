...
from fns import load_svhn, convert_to_all_classes_array, load_original_sentiment_dataset
...
def linear_layer(input_, output_size, name):
    with tf.variable_scope("Linear"):
        # print(input_.get_shape().as_list())
        #weight_matrix = tf.get_variable(name + "_W", [input_.get_shape().as_list()[1], output_size], tf.float32,
        #                               tf.random_normal_initializer(stddev=np.sqrt(input_.get_shape().as_list()[1])))
        weight_matrix = tf.get_variable(name + "_W", [input_.get_shape().as_list()[1]*output_size], tf.float32, tf.truncated_normal_initializer(stddev = 0.1), regularizer = tf.contrib.layers.l2_regularizer(scale = 0.1))
        #bias_vector = tf.get_variable(name + "_b", [output_size], tf.float32, initializer=tf.constant_initializer(0.1))
        #return tf.add(tf.matmul(input_, weight_matrix), bias_vector)
        #weight_matrix = tf.get_variable(name + "_W", [input_.get_shape().as_list()[1], output_size], tf.float32, tf.initializers.he_normal())
                                      # tf.truncated_normal_initializer(stddev = 0.1))
        #weight_matrix = tf.get_variable(name + "_W", [input_.get_shape().as_list()[1], output_size], tf.float32, initializer = tf.contrib.layers.xavier_initializer(uniform=True, seed=None, dtype=tf.float32))
        bias_vector = tf.get_variable(name + "_b", [output_size], tf.float32, initializer=tf.constant_initializer(1))
        return tf.add(tf.matmul(input_, tf.reshape(weight_matrix, [input_.get_shape().as_list()[1], output_size])), bias_vector)
        #return tf.add(tf.matmul(input_, weight_matrix), bias_vector)
...
def fit_model_to_initial_dataset(dataset_name, classifier, model_name, is_resized, is_grayscale, pca = None):
        if(dataset_name == 'mnist' or dataset_name == 'fashion_mnist'):
            ...
            _, _, cv_X, cv_Y, _, _ = load_original_sentiment_dataset()
            cv_X, cv_Y = cv_X[:10], cv_Y[:10]
...
class Neural_Network(object):
    def __init__(self, sess, epochs, batch_size, learning_rate, input_size, output_classes, hidden_layers,
    ...
    def create_tf_model(self, name):
        ...
        for i in range(no_hidden_layers):
            if (i == 0):
                ...
                #outp = BatchNormalization()(outp)
                outp = Activation('relu')(outp)
            outp = Dropout(0.1)(outp, training=self.mc_dropout)
    ...
    def train_model(self, data_X, data_Y, cv_X, cv_Y):
        ...
        history = self.model.fit(data_X, data_Y, epochs=self.epochs, batch_size=self.batch_size, validation_data = (cv_X, cv_Y), shuffle = True, verbose = 0,
    ...
    def apply_fisher_update(self, global_step):
        self.gradients = self.optimizer1.compute_gradients(self.loss, self.train_vars) # returns a list of tensors
        print(tf.shape_n(self.gradients))
        for i in self.gradients:
            print (i)
        pd = CategoricalPd(self.predictions)
        kl = tf.reduce_mean(pd.self_kl())
        all_new_gradients = []
        for i in range(len(self.train_vars)):
            #p = tf.reshape(i, [-1, 1])
            fisher_matrix = tf.hessians(kl, self.train_vars[i])
            print(tf.shape_n(fisher_matrix))
            try:
                fish_inv = tf.matrix_inverse(fisher_matrix)
            except:
                fish_inv = self.pinv(fisher_matrix, self.gradients[i][0])
            new_gradient = tf.matmul(fish_inv[0], tf.reshape(self.gradients[i][0], [-1, 1]))
            print(new_gradient.shape)
            new_gradient = tf.reshape(new_gradient, [-1])
            all_new_gradients.append((new_gradient, self.train_vars[i]))
        self.train_step = self.optimizer1.apply_gradients(all_new_gradients, global_step = global_step)
        
        
    def create_summary_writer(self):
        current_time = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
        train_log_dir = 'logs/Adam/' + current_time + '/train'
        test_log_dir = 'logs/Adam/' + current_time + '/test'
        self.train_summary_writer = tf.summary.create_file_writer(train_log_dir)
        self.test_summary_writer = tf.summary.create_file_writer(test_log_dir)
        
    def create_tf_model_2(self, name):
        with tf.variable_scope(name):
            regularizer = tf.contrib.layers.l2_regularizer(scale = 0.001)
            global_step = tf.Variable(0, trainable=False)
            self.outputs = tf.placeholder(tf.int32, [None, self.output_classes], name="output")
            self.initial_lr_rate = tf.placeholder(tf.float32, (), name="exponentially_decaying_lr")
            #self.lr_rate = tf.train.exponential_decay(self.initial_lr_rate, global_step, 10000, 0.96, staircase=True)
            self.inputs = tf.placeholder(tf.float32, [None, self.input_size], name="input_images")
            self.dropout_rate_var = tf.placeholder_with_default(0.0, (), name="dropout_rate")

            no_hidden_layers = len(self.hidden_layers)

            output = tf.nn.relu(linear_layer(self.inputs, self.hidden_layers[0], "hidden_layer_0"))
            output = tf.nn.dropout(output, keep_prob = (1 - self.dropout_rate_var))
            for i in range(no_hidden_layers - 1):
                output = tf.nn.relu(linear_layer(output, self.hidden_layers[i + 1], "hidden_layer_" + str(i + 1)))
                output = tf.nn.dropout(output, keep_prob = (1 - self.dropout_rate_var))
                
            self.final_layer_vals = output
            self.last_layer = linear_layer(output, self.output_classes, "output_layer")
            self.predictions = tf.nn.softmax(self.last_layer)
        #softmax cross entropy with logits v2 automatically adds the softmax layer    
        self.loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=self.last_layer, labels=self.outputs))
        reg_variables = tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)
        reg_constant = 0.1
        self.loss+=(reg_constant*sum(reg_variables))
        #self.loss+=tf.losses.get_regularization_loss()
        self.train_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=name)
        #self.optimizer = tf.train.AdamOptimizer(learning_rate=self.lr_rate).minimize(self.loss, var_list=train_vars, global_step=global_step)
        #ADAM OPTIMIZER
        self.optimizer1 = tf.train.AdamOptimizer(learning_rate=self.initial_lr_rate).minimize(self.loss, var_list=self.train_vars, global_step=global_step)
        #ADADELTA
        self.optimizer2 = tf.train.AdadeltaOptimizer(learning_rate = self.initial_lr_rate, rho = 0.95, epsilon = 1e-6).minimize(self.loss, global_step = global_step, var_list = self.train_vars)
        #ADAGRAD
        self.optimizer3 = tf.train.AdagradOptimizer(learning_rate = self.initial_lr_rate).minimize(self.loss, global_step = global_step, var_list = self.train_vars)
        #ADAGRAD DA
        #self.optimizer4 = tf.train.AdagradDAOptimizer(self.initial_lr_rate, global_step).minimize(self.loss, global_step, var_list = self.train_vars)
        #GRADIENT DESCENT
        self.optimizer5 = tf.train.GradientDescentOptimizer(learning_rate = self.initial_lr_rate).minimize(self.loss, global_step = global_step, var_list = self.train_vars)
        #MOMENTUM
        self.optimizer6 = tf.train.MomentumOptimizer(learning_rate = self.initial_lr_rate, momentum = 0.9).minimize(self.loss, global_step = global_step, var_list = self.train_vars)
        #MOMENTUM WITH NESTEROV
        self.optimizer7 = tf.train.MomentumOptimizer(learning_rate = self.initial_lr_rate, momentum = 0.9, use_nesterov = True).minimize(self.loss, global_step = global_step, var_list = self.train_vars)
        #RMSProp
        self.optimizer8 = tf.train.RMSPropOptimizer(learning_rate = self.initial_lr_rate).minimize(self.loss, global_step = global_step, var_list = self.train_vars)
        #Fisher Update With Adam Optimizer
        #self.optimizer1 = tf.train.AdamOptimizer(learning_rate=self.initial_lr_rate)
        #self.apply_fisher_update(global_step)
        correct_predictions = tf.equal(tf.arg_max(self.predictions, 1), tf.arg_max(self.outputs, 1))
        self.accuracy = tf.reduce_mean(tf.cast(correct_predictions, tf.float32))
        
        #tf.global_variables_initializer().run()
        #self.create_summary_writer()

    def train_model_2(self, data_X, data_Y, cv_X, cv_Y):
        tf.global_variables_initializer().run()    
        data_X = data_X.reshape(data_X.shape[0], self.input_size)
        data_Y = convert_one_hot(data_Y, self.output_classes)
        no_batches = len(data_X) // (self.batch_size)

        ##print("Training begins.....")
        for epoch in range(self.epochs):
            for index in range(no_batches):
                batch_images = data_X[index * self.batch_size:(index + 1) * self.batch_size]
                batch_images_Y = data_Y[index * self.batch_size: (index + 1) * self.batch_size]
                #TODO::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::ADD CROSS VALIDATION HERE
                _, loss_val, accuracy = self.sess.run([self.optimizer1, self.loss, self.accuracy],
                                                      feed_dict={self.dropout_rate_var: self.train_dropout_rate,
                                                                 self.inputs: batch_images,
                                                                 self.outputs: batch_images_Y,
                                                                 self.initial_lr_rate: self.learning_rate})
                #with self.train_summary_writer.as_default():
                #    tf.summary.scalar('loss', loss_val, step=epoch)
                #    tf.summary.scalar('accuracy', accuracy, step=epoch)

                #print("Epoch: " + str(epoch) + ", Batch " + str(index) + ", Minibatch Loss = " + "{:.4f}".format(loss_val))
                #print("Accuracy on the batch " + str(index) + " is: " + str(accuracy))

        ##print("Training finished")

    def get_predictions_2(self, data_X, with_acc, data_Y):
        #data_X = data_X.reshape(data_X.shape[0], self.input_size)
        if (with_acc):
            if (self.mc_dropout):
                
                loss, probs, acc, final_layer = self.sess.run([self.loss, self.predictions, self.accuracy, self.final_layer_vals],
                                           feed_dict={self.dropout_rate_var: self.dropout_rate, self.inputs: data_X,
                                                      self.outputs: data_Y, self.initial_lr_rate: self.learning_rate})
            else:
                loss, probs, acc, final_layer = self.sess.run([self.loss, self.predictions, self.accuracy, self.final_layer_vals],
                                           feed_dict={self.inputs: data_X, self.outputs: data_Y,
                                                      self.initial_lr_rate: self.learning_rate})
            preds = np.argmax(np.array(probs), axis=-1)
            #with self.test_summary_writer.as_default():
            #    tf.summary.scalar('loss', loss, step=epoch)
            ...
            if (self.mc_dropout):
                probs, final_layer = self.sess.run([self.predictions, self.final_layer_vals], feed_dict={self.dropout_rate_var: self.dropout_rate,
                                                                     self.inputs: data_X,
                                                                     self.initial_lr_rate: self.learning_rate})
            else:
                probs, final_layer = self.sess.run([self.predictions, self.final_layer_vals],
                                      feed_dict={self.inputs: data_X, self.initial_lr_rate: self.learning_rate})
            preds = np.argmax(np.array(probs), axis=-1)
            return probs, preds, final_layer
...
class ensemble_model(object):
    def __init__(self, input_size, epochs, batch_size, output_size, learning_rate, sess, ensembling_technique):
        self.input_size = input_size
        self.epochs = epochs
        self.batch_size = batch_size
    ...
    def get_model(self, model_name, index, dataset_name, is_resized, is_grayscale):
        ...
        if(model_name[0] == 'ann'):
            model = Neural_Network(self.sess, self.epochs, self.batch_size, self.learning_rate, self.input_size,
            ...
            self.pca = None
            #, whiten = True
            model = svm.SVC(C=0.01, kernel='rbf', gamma='auto', probability=True, random_state=0)
            model = fit_model_to_initial_dataset(dataset_name, model, "svm", is_resized, is_grayscale, pca = self.pca)
        
        elif(model_name[0] == 'knn'):
            model = KNeighborsClassifier(30)
            self.pca2 = PCA(n_components = 30, whiten = True)
            model = fit_model_to_initial_dataset(dataset_name, model, "knn", is_resized, is_grayscale, pca = self.pca2)
        
        elif(model_name[0] == "lr"):
            model = LogisticRegr("lbfgs")
            model = fit_model_to_initial_dataset(dataset_name, model, "lr", is_resized, is_grayscale)
