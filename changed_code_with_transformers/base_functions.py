...
import numpy as np
import sys
from fns import load_cifar10_dataset, load_mnist_dataset, load_fashionmnist, find_average, get_frequency_components_dataset, load_stanford40_dataset, load_sentiment_dataset, load_bert_embeddings
...
def get_dataset(dataset_name, is_binarized, is_resized, is_grayscale, sentiment_mode = 'original', classification_type = 'binary'):
    if(dataset_name == "sentiment_analysis"):
        data_X, data_Y, cv_X, cv_Y, test_X, test_Y = load_sentiment_dataset(mode = sentiment_mode, classification_type = classification_type, emb_type = all_globals.embedding_type)
...
def perform_interpretation(mc_dropout, outp_train, outp_cv, outp_test, model_A, model_B):
    ...
    for iters in range(no_repeats):
        ...
        if(all_globals.interpretability_mode == 'counter_factual'):
            probs_train_B_cf, probs_cv_B_cf, probs_test_B_cf = model_B.get_output(mode = all_globals.interpretability_mode)
            interpretability_train, interpretability_cv, interpretability_test = model_A.calculate_interpretability(probs_train_B, probs_cv_B, probs_test_B, probs_B_train_cf = probs_train_B_cf, probs_B_cv_cf = probs_cv_B_cf, probs_B_test_cf = probs_test_B_cf)
        else:
...
def test_sentiment_analysis():
    data_X, data_Y, cv_X, cv_Y, test_X, test_Y = get_dataset(all_globals.dataset_name, all_globals.is_binarized, all_globals.is_resized, all_globals.is_grayscale, classification_type= all_globals.classification_type)
    print(data_X.shape, data_Y.shape, cv_X.shape, cv_Y.shape, test_X.shape, test_Y.shape)
    a = input()
    if(all_globals.interpretability_mode == 'counter_factual'):
        cf_data_X, cf_data_Y, cf_cv_X, cf_cv_Y, cf_test_X, cf_test_Y = get_dataset(all_globals.dataset_name, all_globals.is_binarized, all_globals.is_resized, all_globals.is_grayscale, all_globals.interpretability_mode, classification_type=all_globals.classification_type)    
    no_features = data_X.shape[1]
    #print(data_X.shape, data_Y.shape)
    #data_X, data_Y = data_X[:100], data_Y[:100]
    interpret_train = []
    interpret_test = []
    interpret_cv = []
    for itera in range(3):
        model_B = ModelB(all_globals.model_B_epochs, all_globals.batch_size, all_globals.dataset_name, all_globals.learning_rate, all_globals.model_name_B, all_globals.is_binarized, all_globals.is_resized, all_globals.is_grayscale, no_features)
        model_A = ModelA(all_globals.model_A_epochs, all_globals.batch_size, all_globals.dataset_name, all_globals.learning_rate, all_globals.model_name_A, all_globals.is_binarized, all_globals.is_resized, all_globals.is_grayscale, no_features, mode= all_globals.interpretability_mode, entropy_mode=all_globals.entropy_mode)
        model_B.init_model()
        #X_train, X_cross_validation, Y_train, Y_cross_validation = train_test_split(data_X, data_Y, test_size = 0.2, random_state = 42, stratify = data_Y)
        print("Iteration " + str(itera))
        model_B.set_dataset(data_X, data_Y, test_X, test_Y, cv_X, cv_Y)
        if(all_globals.interpretability_mode == 'counter_factual'):
            model_B.set_counter_factual_dataset(cf_data_X, cf_data_Y, cf_test_X, cf_test_Y, cf_cv_X, cf_cv_Y)
        model_B.train_model()
        ##print("Model B trained")
        model_A.set_dataset(data_X, data_Y, test_X, test_Y, cv_X, cv_Y)
        if(all_globals.interpretability_mode == 'counter_factual'):
            model_A.set_counter_factual_dataset(cf_data_X, cf_data_Y, cf_test_X, cf_test_Y, cf_cv_X, cf_cv_Y)
        outp_train, outp_cv, outp_test = [], [], []
        outp_train, outp_cv, outp_test = perform_interpretation(all_globals.mc_dropout, outp_train, outp_cv, outp_test, model_A, model_B)
        interpret_train.append(find_average(outp_train))
        interpret_test.append(find_average(outp_test))
...
def interpretation_diff_frequencies(no_samples):
