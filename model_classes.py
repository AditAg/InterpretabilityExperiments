    ...
    def __init__(self, epochs, batch_size, dataset_name, learning_rate, model_name, is_binarized, is_resized, is_grayscale, feature_size = None, mode = 'original', entropy_mode = 'original'):
        ...
        if (self.model_name == "svm"):
            if(self.dataset_name == 'sentiment_analysis'):
                ...
                self.classifier = svm.SVC(C = self.param_list['C'], kernel = self.param_list['kernel'], gamma = self.param_list['gamma'], coef0 = self.param_list['coef0'], degree = self.param_list['degree'], probability = self.param_list['probability'], random_state = self.param_list['random_state'], decision_function_shape = 'ovo')
            else:
