...
def apply_vectorizer(data, vectorizer, classification_type, fit = False):
    list_documents = []
    if(classification_type == 'binary'):
        ...
        final_Y = data[:, 1]
        final_Y[final_Y == 'Very negative'] = 0
        final_Y[final_Y == 'Negative'] = 1
        final_Y[final_Y == 'Neutral'] = 2
        final_Y[final_Y == 'Positive'] = 3
        final_Y[final_Y == 'Very positive'] = 4
    
    for i in range(data.shape[0]):
        list_documents.append(data[i][2])
...
def load_sentiment_dataset(mode = 'original', classification_type = 'binary'):
    ...
    if(mode == 'original'):
        tfidf_total_Y, tfidf_total_X = apply_vectorizer(total_data, vectorizer, classification_type, fit = True)       #CV + TRAIN DATA
    else:
        tfidf_total_Y, tfidf_total_X = apply_vectorizer(total_data, vectorizer, classification_type, fit = False)
    ...
    tfidf_test_Y, tfidf_test_X = apply_vectorizer(test_data, vectorizer, classification_type, fit = False)
...
# =============================================================================
# #OTHER POSSIBILITY FOR LOADING IMDB SENTIMENT DATASET
# from keras.datasets import imdb
# (X_train, y_train), (X_test, y_test) = imdb.load_data() #THIS produces a dataset consisting of words which have been replaced by integers representing the absolute popularity of the word in the dataset
#After this, word embeddings should be used to encode the words in the form of real-valued vectors
...
# pyplot.show()
# #Usefully, the imdb.load_data() provides additional arguments including the number of top words to load (where 
#words with a lower integer are marked as zero in the returned data), the number of top words to skip (to avoid the 
#                                                                  “the”‘s) and the maximum length of reviews to support.
# =============================================================================
